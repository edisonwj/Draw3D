...
* 
*/
public class TestRoomScanCoverSelect extends Application {
	private Draw3D dt;
...
//		dt.setYaxisUp(true);
		...
		dt.setCamera(-90.0,  -180.0,  -1000.0);
		dt.setOriginView(-75, 0, 0);
		...
		cameraList.add(new RoomCamera(new Pointd (21,  1), 140));
		...
		int ci = 0;
		for (RoomCamera camera : cameraList) {
			...
			dt.drawLabel(-10, 39, 0, ("Case #"+ ci +
									 ", Orientation= " + camera.orientation +
									 ", Location= " + camera.location.getx() +
									 ", " + camera.location.gety()) );
			dt.drawLabel(-10, 36, 0, ("inWallCount= " + inWallCount +
									 ", coveredCount= " + coveredCount +
									 ", notCoveredCount= " + notCoveredCount) );
			dt.drawLabel(-10, 33, 0, ("size= " + size +
					 				  ", check= " + check) );
...
//			System.out.println("inWall " + wallNum + " : " + start + ", " + end);
	...
	private RoomCamera[] createCameraList() {
		// Define Cameras
		RoomCamera[] cameraList = {
				new RoomCamera(new Pointd ( 1, 21), 310),
				new RoomCamera(new Pointd ( 6, 12),  90),
				new RoomCamera(new Pointd (21,  1), 140),
				new RoomCamera(new Pointd (11, 21), 310)
		};
		return cameraList;
	}
